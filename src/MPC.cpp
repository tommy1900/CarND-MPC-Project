...
// prediction horizon
size_t N = 15; // since at time 0, its instance t - 1
...
double cte_w = 85.0; // too high may cause over-correct 100
double epsi_w = 45.0; // 30
double v_w = 0.4; // 0.3
double delta_w = 5500.0; // increase this when high osilation 5500
double acc_w = 1.0; // 1
double delta_dot_w = 50.0; // increase this when high osilation 50
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
     ...
     */
     // NOTE: Additionally math functions must be called from CppAD
      ...
      double track_error_weight = cte_w;
      double steering_error_weight = epsi_w;
      double velocity_error_weight = v_w;

      double steer_weight = delta_w;
      double accel_weight = acc_w;

      double steer_change_weight = delta_dot_w;
      double accel_change_weight = acc_dot_w;

      for (size_t t = 0; t < N; t++) {
        // penalty for the tracking error
        ...
        /*if(t == N-1){
        cout<< "cte: " << track_error_weight* CppAD::pow(vars[cte_start + t], 2)
        << "epsi: " << steering_error_weight* CppAD::pow(vars[epsi_start + t], 2)
        << "vel:  " << velocity_error_weight* CppAD::pow(vars[v_start + t] - ref_v, 2) << endl;}*/
      }
      for (size_t t = 0; t < N-1; t++) {
        ...
        /*if(t == N-2){
        cout<< "steer_force: " << steer_weight * CppAD::pow(vars[delta_start + t], 2)
        << "accel_force: " << accel_weight * CppAD::pow(vars[a_start + t], 2)
        << "the same time:  " << 300*CppAD::pow(vars[delta_start + t] * vars[v_start + t], 2) << endl;}*/
...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs) {
  ...
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = delta_min; // since the kinematic : dpsi/dt = delta/L*v
